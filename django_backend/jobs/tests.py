...
class JobModelTestCase(TestCase):
    def setUp(self):
        resume_file = self.create_file_from_path(
            os.path.join(os.path.dirname(__file__), "data/MahmoudBarzegar.pdf"), "MahmoudBarzegar.pdf"
        )
        self.resume = ResumeModel.objects.create(file=resume_file, resume_info={})
